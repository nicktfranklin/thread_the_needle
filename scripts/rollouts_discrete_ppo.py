...
from model.agents import DiscretePPO as PPO
...
BASE_FILE_NAME = "thread_the_needle_discrete_ppo"
...
def main():
    ...
    for ii in range(config.n_batch):
        ...
        with open(
            f"{config.results_dir}discrete_ppo_batched_data_{date.today()}.pkl",
            "wb",
        ) as f:
    ...
    with open(f"{config.results_dir}discrete_ppo_batched_data.pkl", "wb") as f:
        pickle.dump(batched_data, f)

...
from src.model.state_inference.gumbel_softmax import gumbel_softmax
from src.model.state_inference.nets.cnn import AutoregressiveDeconvNet
...
class BaseVaeFeatureExtractor(BaseFeaturesExtractor, ABC):
    ...
    def get_states(self, observations: torch.Tensor) -> torch.LongTensor:
        features = self._extract_features(observations)
        logits = self._encode(features)
        latents = self.reparameterize(logits).view(-1, self.z_layers, self.z_dim)
        return torch.argmax(latents, dim=-1)
...
class DiscreteVaeExtractor(BaseVaeFeatureExtractor):
    ...
    to be used with a VAE for discrete observations.
